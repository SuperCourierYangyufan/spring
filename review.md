...
6. ThreadPoolExecutor回收工作线程的原理
    1. ThreadPoolExecutor回收工作线程，一条线程getTask()返回null，就会被回收
    2. 未调用shutdown() ，RUNNING状态下全部任务执行完成的场景
        - 线程数量大于corePoolSize，线程超时阻塞
        - 超时唤醒后CAS减少工作线程数，如果CAS成功，返回null，线程回收
        - 否则进入下一次循环。当工作者线程数量小于等于corePoolSize，就可以一直阻塞了
    3. 调用shutdown() ，全部任务执行完成的场景,shutdown() 会向所有线程发出中断信号，这时有两种可能。
        * 所有线程都在阻塞
            - 中断唤醒，进入循环，都符合第一个if判断条件，都返回null，所有线程回收
        * 任务还没有完全执行完
            - 至少会有一条线程被回收
            - 会调用tryTerminate()，向任意空闲线程发出中断信号
...
5. I/O模型
    ...
    * 如果小明点击下载按钮之后，就一直干瞪着进度条不做其他任何事情直到软件下载完成，但是软件下载完成其实是会「叮」的一声通知的  
    （但小明依然那样干等着），这是异步阻塞；（不常见）
...
7. Http
    *  Http与Https的区别
        ...
        7. HTTP协议运行在TCP之上，所有传输的内容都是明文，HTTPS运行在SSL/TLS之上，SSL/TLS运行在TCP之上，所有传输的内容都经过加密的
    * HTTPS工作原理
    ...
    * Http1.1
       ...
       2. 带宽优化及网络连接的使用: 请求头引入了range头域，它允许只请求资源的某个部分，即返回码是206
       3. 错误通知的管理: 在HTTP1.1中新增了24个错误状态响应码
       4. Host头处理: 在HTTP1.0中认为每台服务器都绑定一个唯一的IP地址，但随着虚拟主机技术的发展，在一台物理服务器上可以存在多个虚拟主机,    
       并且它们共享一个IP地址,HTTP1.1的请求消息和响应消息都应支持Host头域，且请求消息中如果没有Host头域会报告一个错误
       5. 长连接: 在一个TCP连接上可以传送多个HTTP请求和响应，减少了建立和关闭连接的消耗和延迟
    * HTTP2.0
        1. 新的二进制格式: HTTP1.x的解析是基于文本,基于这种考虑HTTP2.0的协议解析决定采用二进制格式，实现方便且健壮
        ...
        4. 服务端推送: 例如我的网页有一个sytle.css的请求，在客户端收到sytle.css数据的同时，服务端会将sytle.js的文件推送给客户端，  
...
11. 死信队列
    * RocketMQ中消息重试超过一定次数后（默认16次）就会被放到死信队列中，在消息队列RocketMQ 中，这种正常情况下无法被消费的消息称为死信消息
    * 可以在控制台Topic列表中看到“DLQ”相关的Topic,死信队列也可以被订阅和消费，并且也会过期
...
3. InnoDB构成
    ...
    使用查询缓存，并在MySQL 8.0中删除
        ...
        * 缓冲区淘汰原则采用LRU(最久未使用淘汰),存放一个链表,最新使用的放入表头.整个链+表分为两节,一节储存使用率不高的old区  
...
11. 日志
    ...
    * binlog
        1. 它记录了所有的 DDL 和 DML 语句（除了数据查询语句select、show等），以事件形式记录，还包含语句所执行的消耗的时间
        2. MySQL的二进制日志是事务安全型的
        3. binlog 的主要目的是复制和恢复
12. 主从
    * 步骤
    ...
    * 问题
        ...
        2. 从库只有一个SQL Thread，主库写压力大，复制很可能延时
            - 并行复制----解决从库复制延迟的问题
            - 在从库中有两个线程IO Thread和SQL Thread，都是单线程模式工作，因此有了延迟问题，我们可以采用多线程机制来加强，减少从库复制延迟     
12. 锁
...
2. 自动装配
    ...
    * ASpringBootApplicationutoConfigurationPackages.Registrar中取主启动类所在包及子包下的组件设置为basePackage.
...
7. 循环依赖
    ...
    * 失效
        - 互相循环依赖使用构造器注入
        - 通过类似 @Async 注解在对象初始化时期被生成代理对象替换原对象原因
8. AOP
...
5. Es写入过程  
    ...
    4. 写入过程是文档进行处理后不直接写入磁盘,而是写入内存缓存区,方便查询,也是降低消耗 
    4. 每30分钟或当缓冲区达到一定大小才会flush到磁盘,为了防止down机丢数据,所以引入TransLog
...
6. IK分词器,默认有个词的树,以一个个字母或者汉字为单位,每个词就是一个节点,每个节点下面有可通的词,然后能走通代表为一个词,  
    然后句子进来一个个字的走判断词是否可达.
        - 设置ik_max_word为最细粒度拆分,ik_smart为最粗粒度拆分
        - custom.dic可以配置自定义的词汇
...
10. es的倒排索引是不可修改的,为了解决动态问题,更新和删除的数据会加上.del后缀,查询时会全部查出,然后过滤返回.  
    一段时间后新的索引会覆盖旧索引
        - 好处1,不需要锁,因为不更新索引,没有竞争
        - 好处2 倒排索引会放入内存,因为不变性,所以大部分时间可以直接访问内存,不会命中磁盘
        - 好处3 在索引的生命周期内始终有效,不会因为数据修改导致索引更变,消耗性能
        - ps1 新增的数据也会放入内存,写translog
        - ps2 一个文档多次修改在同一个段,准确来说,倒排索引中单词指向的就是一个段
11. es分片数据备份是异步多线程的,所以会有并发问题,es采用乐观锁,老版本用version,新版本为_seq_no+_primary_term
12. 优化
    - 硬盘优化,SSD,使用多块硬盘,Es可以配置path.data目录配置分配到多个上面
    - 合理的分片,过少分片,会导致并行能力不够好,过多,每个分片都需要消耗资源,且同一个节点下的分片都会竞争资源.
    - 查询最好带ID,不做深翻页
    - 新增数据时Lucene默认每1s从内存刷新到文件缓存中,如果实时性不高,可以将Refresh周期延长,提高写的效率
    - 内存分配 Xmx=Xms,是为了Java垃圾回收后,堆不需要重新分配计算堆的大小而浪费资源,内存大小最好不要超过32G,且为物理内存一半,
    因为Es底层Lucene,会将一部分数据放入缓存,方便访问
13. master选举,对所有可以成为master的节点(配置项中)根据nodeId排序,每个节点排序一次,第0位暂定位为该节点投票的master节点,
    当票数为节点数/2+1(需要配置,防止脑裂)的节点成为master节点(彼此通过RPC发现彼此)。
