...
4. InnoDb逻辑存储结构主要包括表空间，段，区，页，行组成
    ...
    4. 页是MySQL中磁盘和内存交互的基本单位，也是MySQL是管理存储空间的基本单位，默认大小为16KB。一次读写最小也是16kb
        ...
        * 页结构
            1. FileHeader,文件头部,页的一些通用信息
            2. PageHeader,页面头部,数据页专有的一些信息
            3. Infimum + Supremum,最小记录和最大记录,两个虚拟的行记录
            4. UserRecords,用户记录,实际存储的行记录内容
            5. FreeSpace,空闲空间,页中尚未使用的空间
            6. PageDirectory,页面目录,页中的某些记录的相对位置
            7. FileTrailer,文件尾部,校验页是否完整	
